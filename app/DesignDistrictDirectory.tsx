...
import { Search, Star, StarOff, Globe2, Building2, Rows, Grid2X2, Filter, X, ChevronDown } from "lucide-react";
      ...
      {options.map((opt) => (
        <button
          ...
          className={classNames(
            ...
            value === opt.value ? "bg-white shadow-sm dark:bg-zinc-900" : "text-zinc-600 hover:text-zinc-900 dark:text-zinc-300 dark:hover:text-white"
          )}
        >
          {opt.label}
        </button>
...
function Dropdown({ label, value, setValue, items }: { label: string; value?: string; setValue: (v?: string) => void; items: string[] }) {
  const [open, setOpen] = useState(false);
  return (
    <div className="relative">
      <button onClick={() => setOpen((o) => !o)} className="inline-flex items-center gap-2 rounded-xl border border-zinc-300 dark:border-zinc-700 px-3 py-2 text-sm hover:bg-zinc-50 dark:hover:bg-zinc-800">
      ...
      <AnimatePresence>
        {open && (
          <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} className="absolute z-20 mt-2 w-48 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 shadow-lg">
            <button className="w-full text-left px-3 py-2 text-sm hover:bg-zinc-50 dark:hover:bg-zinc-800" onClick={() => { setValue(undefined); setOpen(false); }}>
              Alle
            </button>
            <div className="max-h-64 overflow-auto">
              {items.map((it) => (
                <button key={it} className="w-full text-left px-3 py-2 text-sm hover:bg-zinc-50 dark:hover:bg-zinc-800" onClick={() => { setValue(it); setOpen(false); }}>
                  {it}
                </button>
              ))}
            </div>
...
function DirectoryCard({ item, favorite, toggleFavorite, layout }: { item: DirectoryItem; favorite: boolean; toggleFavorite: (id: string) => void; layout: "grid" | "rows" }) {
  return (
    <motion.article layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="group relative overflow-hidden rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900">
      <div className="p-4 flex flex-col gap-3">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <h3 className="truncate text-base font-semibold">{item.name}</h3>
            <div className="mt-0.5 flex items-center gap-2 text-xs text-zinc-500">
              <span className="inline-flex items-center gap-1"><Globe2 className="h-3.5 w-3.5" /> {item.country}{item.city ? `, ${item.city}` : ""}</span>
              <span className="inline-flex items-center gap-1"><Building2 className="h-3.5 w-3.5" /> {item.type}</span>
            </div>
          </div>
          <button onClick={() => toggleFavorite(item.id)} className="p-2 rounded-lg border border-zinc-200 hover:bg-zinc-50 dark:border-zinc-700 dark:hover:bg-zinc-800" aria-label={favorite ? "Fjern favorit" : "Tilføj favorit"}>
            {favorite ? <Star className="h-4 w-4 fill-current" /> : <StarOff className="h-4 w-4" />}
          </button>
        </div>

        {item.description && <p className="line-clamp-2 text-sm text-zinc-600 dark:text-zinc-300">{item.description}</p>}

        {item.url && (
          <div className="pt-2">
            <a href={item.url} target="_blank" rel="noopener noreferrer" className="inline-flex text-sm font-medium underline underline-offset-4">Besøg website</a>
          </div>
        )}
      </div>
    </motion.article>
...
export default function DesignDistrictDirectory() {
  ...
  const [favorites, setFavorites] = useState<string[]>(() => {
    ...
    try { return JSON.parse(localStorage.getItem("ddcph:favorites") || "[]"); } catch { return []; }
  });
  ...
  const items = useMemo(() => {
    return MOCK_DATA
      ...
      .filter((it) => (query ? fuzzyIncludes(it.name, query) || fuzzyIncludes(it.country, query) || fuzzyIncludes(it.city || "", query) : true))
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-zinc-600 dark:text-zinc-300 max-w-2xl">En hurtig og elegant måde at udforske {mode.toLowerCase()} på tværs af lande.</p>
        </div>
        <Segmented value={mode} onChange={(v) => setMode(v as any)} options={[{ label: "Udstillere", value: "Udstillere" }, { label: "Brands", value: "Brands" }]} />
      </div>

      <div className="mt-6 grid grid-cols-1 gap-3 md:grid-cols-12">
        <div className="md:col-span-5">
          <label className="sr-only" htmlFor="search">Søg</label>
          <div className="flex items-center gap-2 rounded-xl border border-zinc-300 dark:border-zinc-700 px-3 py-2">
            <Search className="h-4 w-4 opacity-60" />
            <input id="search" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Søg efter navn eller land…" className="w-full bg-transparent outline-none text-sm" />
          </div>
        </div>
        <div className="md:col-span-7">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {(country || query) && (
              <button onClick={() => { setCountry(undefined); setQuery(""); }} className="inline-flex items-center gap-2 rounded-xl border border-zinc-300 dark:border-zinc-700 px-3 py-2 text-sm hover:bg-zinc-50 dark:hover:bg-zinc-800">
            ...
            <span className="ml-auto inline-flex items-center gap-2 rounded-xl border border-zinc-300 dark:border-zinc-700 px-3 py-2 text-sm">
              Visning:
              <button onClick={() => setLayout("grid")} className={classNames("p-1 rounded-md", layout === "grid" ? "bg-zinc-100 dark:bg-zinc-800" : "hover:bg-zinc-50 dark:hover:bg-zinc-800")} aria-label="Grid">
                <Grid2X2 className="h-4 w-4" />
              </button>
              <button onClick={() => setLayout("rows")} className={classNames("p-1 rounded-md", layout === "rows" ? "bg-zinc-100 dark:bg-zinc-800" : "hover:bg-zinc-50 dark:hover:bg-zinc-800")} aria-label="Rows">
      ...
      <div className={classNames("mt-4 gap-4", layout === "grid" ? "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3" : "grid grid-cols-1")}>
        <AnimatePresence mode="popLayout">
          {items.map((it) => (
            <DirectoryCard key={it.id} item={it} favorite={favorites.includes(it.id)} toggleFavorite={toggleFavorite} layout={layout} />
          ))}
        </AnimatePresence>
      </div>
    </div>
